<template>
  <AdminLayout>
    ...
    <div class="flex flex-col min-h-[calc(100vh-180px)]">
      ...
      >
        <MedicosTable ref="tabela" :atualizar="atualizar" @selecionado-change="onSelecionadoChange" />
      </ComponentCard>
    </div>
    <MedicoModal :isOpen="modalAberto" :dados="null" @close="fecharModal" />
  </AdminLayout>
</template>
...
import MedicosTable from '@/components/tables/medicos/MedicosTable.vue'
import MedicoModal from '@/components/forms/medicos/MedicoModal.vue'

const currentPageTitle = ref('Médicos')
const modalAberto = ref(false)
const atualizar = ref(false)
const tabela = ref<InstanceType<typeof MedicosTable> | null>(null)
