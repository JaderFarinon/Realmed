<template>
  <AdminLayout>
    ...
    <div class="flex flex-col min-h-[calc(100vh-180px)]">
      <ComponentCard
        title="Motivos de Pendência"
        class-name="flex flex-col flex-1 mb-[15px]"
        body-class="flex flex-col flex-1 min-h-0"
        content-class="flex flex-col flex-1 min-h-0"
      >
        <MotivosPendenciaTable ref="tabela" :atualizar="atualizar" />
      </ComponentCard>
    </div>
    <MotivoPendenciaModal :is-open="modalAberto" :dados="null" @close="fecharModal" />
  </AdminLayout>
</template>
...
const currentPageTitle = ref('Motivos de Pendência')
const modalAberto = ref(false)
const atualizar = ref(false)
const tabela = ref<InstanceType<typeof MotivosPendenciaTable> | null>(null)
