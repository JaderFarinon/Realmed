...
{
    public partial class DialogPreProcessamento
    {
        //WebService
        ...
        private async void GravaPreProcessamento() 
        {
            try
            {
                httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", await localStorage.GetItemAsStringAsync("token"));
                NexusWS _ws = new NexusWS(wsConfig.GetUrl(), httpClient);
                notificacao.DsRespProc = usuario.Nm_usuario;

                if (Disabled == "card-inactive") // Se a notificação não foi editada, será gravado o pré processamento. 
                {
                    statusfunc = await _ws.GravarPreProcessamentoAsync(notificacao);
                    ...
                    {
                        ShowNotification(new NotificationMessage { Severity = NotificationSeverity.Error, Summary = "Erro", Detail = $"Erro: {statusfunc.DsReturn}", Duration = 10000 });
                    }
                }
                else // Se a notificação foi editada, será feito um update da notificacao e em seguida será gravado o pré processamento
                {
                    statusfunc = await _ws.AtualizaNotificacaoIncidenteAsync(notificacao);

                    if (statusfunc.Status)
                    {
                        ShowNotification(new NotificationMessage { Severity = NotificationSeverity.Success, Summary = "Sucesso", Detail = $"{statusfunc.DsReturn}", Duration = 4000 });
                        await _ws.GravarPreProcessamentoAsync(notificacao);
                    }
                    else
                    {
                        ShowNotification(new NotificationMessage { Severity = NotificationSeverity.Error, Summary = "Erro", Detail = $"Erro: {statusfunc.DsReturn}", Duration = 10000 });
