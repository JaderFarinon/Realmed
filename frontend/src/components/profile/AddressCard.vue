  ...
  <div class="mb-0">
    <div class="rounded-2xl border border-gray-200 p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
      <div class="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        <div class="w-full">
          <h4 class="text-lg font-semibold text-gray-800 dark:text-white/90 lg:mb-6">Endereço</h4>

          <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-20">
            <div>
              <p class="mb-1 text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">CEP</p>
              <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ person?.zip_code || '—' }}</p>
            </div>
            <div>
              <p class="mb-1 text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Rua</p>
              <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ person?.street || '—' }}</p>
            </div>
            <div>
              <p class="mb-1 text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Número</p>
              <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ person?.number || '—' }}</p>
            </div>
            <div>
              <p class="mb-1 text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Bairro</p>
              <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ person?.neighborhood || '—' }}</p>
            </div>
            <div>
              <p class="mb-1 text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Cidade</p>
              <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ person?.city || '—' }}</p>
            </div>
            <div>
              <p class="mb-1 text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Estado</p>
              <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ person?.state || '—' }}</p>
            </div>
          </div>
        </div>
    ...
    <Modal v-if="isModalOpen" @close="closeModal">
      <template #body>
        <div
          class="no-scrollbar relative w-full max-w-[640px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-10"
        >
          <button
            @click="closeModal"
          ...
          <div class="px-2 pr-14">
            ...
            <p class="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Atualize os dados de endereço utilizados para contato e cadastro.
            </p>
          </div>
            ...
            <div class="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
              ...
              <div>
                ...
                <input
                  ...
                  type="text"
                  maxlength="2"
                  class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs uppercase focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-700"
                  placeholder="UF"
                />
              </div>
            </div>
...
const props = defineProps<{
  person: (AddressFields & { [key: string]: any }) | null
  isSaving: boolean
  updateToken: number
}>()

const emit = defineEmits<{
  (e: 'update-address', payload: Partial<AddressFields>): void
}>()
...
const syncForm = () => {
  form.zip_code = props.person?.zip_code ?? ''
  form.street = props.person?.street ?? ''
  form.number = props.person?.number ?? ''
  form.neighborhood = props.person?.neighborhood ?? ''
  form.city = props.person?.city ?? ''
  form.state = props.person?.state ?? ''
}
