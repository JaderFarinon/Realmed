...
const CHAT_MENSAGENS_TABLE = 'esteira_procedimento_etapa_chat_mensagens'
const CHAT_ANEXOS_TABLE = 'esteira_procedimento_etapa_chat_anexos'
...
exports.up = async function up(knex) {
  ...
  if (!mensagensExists) {
    await knex.schema.createTable(CHAT_MENSAGENS_TABLE, (table) => {
      ...
      table
        .foreign('solicitacao_etapa_id', 'fk_chat_msg_sol_etapa')
        .references('id')
      ...
      table.index(['solicitacao_etapa_id'], 'idx_chat_msg_etapa')
    })
  }
  ...
  if (!anexosExists) {
    await knex.schema.createTable(CHAT_ANEXOS_TABLE, (table) => {
      ...
      table
        .foreign('chat_mensagem_id', 'fk_chat_anexo_msg')
        .references('id')
      ...
      table.index(['chat_mensagem_id'], 'idx_chat_anexo_mensagem')
      table.index(['deleted_at'], 'idx_chat_anexo_deleted')
    })
  }
}
