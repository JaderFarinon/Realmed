...
const STATUS_ENUM_SQL = "ENUM('PENDENTE','BLOQUEADA','ATRIBUIDO','EM_ANDAMENTO','PARADO','ATRASADO','CONCLUIDO','CANCELADO')"
const STATUS_ENUM_SQL_PREVIOUS = "ENUM('PENDENTE','ATRIBUIDO','EM_ANDAMENTO','PARADO','ATRASADO','CONCLUIDO','CANCELADO')"

const STATUS_PENDENTES_NOVO = "('PENDENTE', 'BLOQUEADA', 'ATRIBUIDO')"
const STATUS_EM_ANDAMENTO = "('EM_ANDAMENTO', 'PARADO', 'ATRASADO')"
const STATUS_PENDENTES_ANTERIOR = "('PENDENTE', 'ATRIBUIDO')"
...
const createResumoEtapasView = async (knex, solicitacoesTable, etapasTable, stageTable, { statusPendentes }) =>
  knex.schema.raw(
    `CREATE VIEW ?? AS
    SELECT
      ...
      SUM(CASE WHEN se.status IN ${statusPendentes} THEN 1 ELSE 0 END) AS etapas_pendentes,
      SUM(CASE WHEN se.status IN ${STATUS_EM_ANDAMENTO} THEN 1 ELSE 0 END) AS etapas_em_andamento,
...
const createResumoEtapasViewSemTabelaEtapas = async (knex, solicitacoesTable, etapasTable, { statusPendentes }) =>
  knex.schema.raw(
    `CREATE VIEW ?? AS
    SELECT
      sc.id AS solicitacao_id,
...
const createResumoEtapasViewLegacy = async (knex, solicitacoesTable, etapasTable) =>
  knex.schema.raw(
    ...
    SELECT
      ...
      MAX(CASE WHEN se.etapa = 'LIBERACAO_GUIA' AND se.status = 'CONCLUIDO' THEN 1 ELSE 0 END) AS liberacao_guia_concluida,
      MAX(CASE WHEN se.etapa = 'LIBERACAO_OPME' AND se.status = 'CONCLUIDO' THEN 1 ELSE 0 END) AS liberacao_opme_concluida,
      SUM(CASE WHEN se.status IN ${STATUS_PENDENTES_ANTERIOR} THEN 1 ELSE 0 END) AS etapas_pendentes,
      SUM(CASE WHEN se.status IN ${STATUS_EM_ANDAMENTO} THEN 1 ELSE 0 END) AS etapas_em_andamento,
      SUM(CASE WHEN se.status = 'CONCLUIDO' THEN 1 ELSE 0 END) AS etapas_concluidas
    FROM ?? sc
    LEFT JOIN ?? se ON se.solicitacao_id = sc.id
  ...
  if (await hasView(knex, VIEW_NAME)) {
    ...
    if (stageTable) {
      await createResumoEtapasView(knex, solicitacoesTable, etapasTable, stageTable, {
        statusPendentes: STATUS_PENDENTES_NOVO,
      })
    } else {
...
exports.down = async function down(knex) {
  if (await hasTable(knex, TABLE_NAME)) {
    await knex.schema.raw(
      `ALTER TABLE ?? MODIFY COLUMN ?? ${STATUS_ENUM_SQL_PREVIOUS} NOT NULL DEFAULT 'PENDENTE'`,
      [TABLE_NAME, 'status'],
    )
  }
  ...
  if (await hasView(knex, VIEW_NAME)) {
    ...
    if (stageTable) {
      await createResumoEtapasView(knex, solicitacoesTable, etapasTable, stageTable, {
        statusPendentes: STATUS_PENDENTES_ANTERIOR,
      })
    } else {
      await createResumoEtapasViewSemTabelaEtapas(knex, solicitacoesTable, etapasTable, {
        statusPendentes: STATUS_PENDENTES_ANTERIOR,
