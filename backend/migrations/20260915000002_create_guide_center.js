...
// MySQL commits DDL statements implicitly. If a migration is interrupted, the
// tables created before the failure remain even though Knex does not record the
// migration as completed. Make each step resumable so migrate:latest can safely
...
exports.up = async (knex) => {
  await createTableIfMissing(knex, 'insurance_providers', (t) => {
  ...
  await createTableIfMissing(knex, 'professionals', (t) => {
  ...
  await createTableIfMissing(knex, 'procedures', (t) => {
  ...
  await createTableIfMissing(knex, 'patients', (t) => {
  ...
  await createTableIfMissing(knex, 'patient_insurances', (t) => {
  ...
  await createTableIfMissing(knex, 'guide_processes', (t) => {
    t.increments('id').primary(); t.integer('patient_id').unsigned().notNullable().references('id').inTable('patients').onDelete('RESTRICT')
  ...
  await createTableIfMissing(knex, 'guide_process_documents', (t) => {
  ...
  const replacementForeignKey = await knex('information_schema.KEY_COLUMN_USAGE')
    .where({
      TABLE_SCHEMA: knex.client.config.connection.database,
      TABLE_NAME: 'guide_process_documents',
      COLUMN_NAME: 'replaced_by_document_id',
      REFERENCED_TABLE_NAME: 'guide_process_documents',
    })
    .first()
  if (!replacementForeignKey) {
  ...
  await createTableIfMissing(knex, 'guide_authorizations', (t) => {
  ...
  await createTableIfMissing(knex, 'guide_process_pending_items', (t) => {
  ...
  await createTableIfMissing(knex, 'guide_process_history', (t) => {
